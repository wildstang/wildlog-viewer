...
public class ScrollBarPanel extends JPanel
{
    ...
    * 
    * Normally the position only ranges from (min, max - extent). We map it to
    * the total range to simplify calculations.
    * 
    * @return mapped scroll position
    */
   public int getScrollPosition()
   {
      return (int) (((double) scrollBar.getValue() / (((double) scrollBar.getMaximum() - (double) scrollBar.getMinimum()) - (double) scrollBar.getModel().getExtent())) * ((double) scrollBar.getMaximum() - (double) scrollBar.getMinimum()));
   }
